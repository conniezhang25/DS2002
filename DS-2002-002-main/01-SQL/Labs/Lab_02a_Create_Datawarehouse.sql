...
#status id joins with order status, replace status id to status name
...
CREATE TABLE `fact_orders` (
  ...
  #KEY `fk_orders_status` (`order_status`)
 # CONSTRAINT `fk_orders_customers` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`),
 # CONSTRAINT `fk_orders_employees1` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`id`),
 # CONSTRAINT `fk_orders_status1` FOREIGN KEY (`status_id`) REFERENCES `orders_status` (`id`),
 # CONSTRAINT `fk_orders_tax_status1` FOREIGN KEY (`tax_status_id`) REFERENCES `orders_tax_status` (`id`),
...
#join order and order status, and order details and order status details, then join orders and order details
